...
function Fetch-And-Save-CVEData($url, $dateProperty, $suffix) {
    try {
        $response = Invoke-RestMethod -Uri $url -Headers $headers -Method Get
        if ($response -ne $null -and $response.vulnerabilities.Count -gt 0) {
            ...
            if ($latestDate) {
                ...
                Write-Host "CVEデータを $filePath に保存しました。"
            } else {
                Write-Host "CVEデータの最新日付を取得できませんでした。"
            }
        } else {
            Write-Host "CVEデータの取得に失敗するか、脆弱性が見つかりませんでした。"
        }
    } catch {
...
Fetch-And-Save-CVEData $urlModified "lastModified" "modified"
...
Fetch-And-Save-CVEData $urlRecent "published" "recent"
